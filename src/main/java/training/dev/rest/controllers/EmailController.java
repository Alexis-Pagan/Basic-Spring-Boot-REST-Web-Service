...
public class EmailController {
	 ...
	 */
	@GetMapping(path="/users/emails-addresses/emailController", produces = {MediaType.APPLICATION_JSON_VALUE}, consumes = {MediaType.APPLICATION_JSON_VALUE}) 
	public ResponseEntity<ArrayList<String>> readEmailAddress() {
		 ...
		 */
		return ResponseEntity.status(200).headers(httpHeader).body(MySQL);
	}
	...
	public ResponseEntity<Response> createEmailAddress(@RequestBody User user) {
		...
		return ResponseEntity.status(HttpStatus.CREATED).headers(header).body(report);
	}
}
